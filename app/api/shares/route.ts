...
export async function POST(request: Request) {
  ...
  const { error } = await supabase.from('shared_jobs').insert({ job_id: job.id, user_id: user.id, token, title: job.title, intro: body?.intro ?? 'قائمة المرشحين المختارة للمراجعة.', candidate_ids: candidateIds })
